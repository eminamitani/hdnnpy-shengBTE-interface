...
#change the read_force to read forces from prediction_result.npz
def read_forces():
    '''
    parse prediction results from hdnnpy
    default output npz name: prediction_result.npz
    here I assume prediction_result.npz placed on the same directory and contain
    energy and force data for all displacement required by shengBTE
    with the same ordering generated by calc_frange
    '''
