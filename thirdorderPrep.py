...
strdim=str(args.dim[0])+" "+str(args.dim[1])+ " "+str(args.dim[2])
cutoff=str(-3)

#generate POSCAR-XXXX files
disp_generate_command='thirdorder_vasp.py'+" sow" +" "+strdim +" "+ cutoff
gd=os.system(disp_generate_command)

#gather all displacement files to disp.xyz
prefix = args.prefix
poscars_init = glob.glob('3RD.POSCAR.[0-9]*')
xyz = 'disp.xyz'
